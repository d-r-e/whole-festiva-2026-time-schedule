...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; background:linear-gradient(130deg,#0e1310,var(--glow)); color:var(--text); font:15px/1.35 "DM Sans",system-ui,sans-serif; }
    ...
    .event.has-listeners { --heat-background:hsl(var(--heat-hue) var(--heat-saturation) var(--heat-light)); background:var(--heat-background); border-color:hsl(var(--heat-hue) var(--heat-saturation) calc(var(--heat-light) + 12%)); color:#fff; } .event.has-listeners:hover { filter:brightness(1.12); } @supports (color:contrast-color(red)) { .event.has-listeners { color:contrast-color(var(--heat-background)); } }
    .event-card { flex:1; min-width:0; width:100%; padding:5px 5px 5px 7px; color:inherit; background:transparent; border:0; border-radius:0; text-align:left; font:inherit; cursor:pointer; } .event-card:hover { border:0; }
    .event.selected { background:var(--accent); color:var(--accent-dark); box-shadow:none; font-weight:700; }
...
function renderSchedule() {
  ...
  dayEvents.forEach(e=>{
    ...
    el.innerHTML=`<button class="event-card"><span class="name">${escape(e.artist)}</span></button>`;
    el.querySelector('.event-card').onclick=()=>showDetail(e); grid.append(el);
  });
